...
const CLAUDE_KEY_STORAGE = 'asproite_claude_key';
...
/* ── Build system prompt from live site data ─────────────────── */
...
export default function AsproiteAI() {
  ...
  const [apiKey] = useState(() => localStorage.getItem(CLAUDE_KEY_STORAGE) || '');
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: newMessages.slice(-10), // keep last 10 for context
        }),
      });

      if (!res.ok) throw new Error(`API error ${res.status}`);
      const result = await res.json();
      const reply = result.content?.[0]?.text || "I'm sorry, I didn't get a response. Please try again or contact us directly at info@asproite.com";
