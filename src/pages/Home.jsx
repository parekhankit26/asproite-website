  ...
  return (
    <>
      ...
      <section style={{ position:'relative', minHeight:'100vh', display:'flex', alignItems:'center', padding:'120px 0 80px', overflow:'hidden' }}>
        <canvas ref={canvasRef} style={{ position:'absolute', inset:0, width:'100%', height:'100%', opacity:0.65, pointerEvents:'none' }} />
        ...
        <div className="hero-inner-content" style={{ position:'relative', zIndex:2, maxWidth:750, padding:'0 64px' }}>
          <div className="hero-typing-tag" style={{ display:'inline-flex', alignItems:'center', gap:10, background:'rgba(0,212,255,0.07)', border:'1px solid var(--border)', borderRadius:100, padding:'6px 18px', fontSize:'0.72rem', letterSpacing:'0.15em', textTransform:'uppercase', color:'var(--cyan)', marginBottom:28, animation:'fadeUp 0.8s ease both', maxWidth:'100%', overflow:'hidden' }}>
            <span className="tag-dot" />
            <span style={{ minWidth:220 }}>{typedText}<span style={{ animation:'blink 0.8s infinite' }}>|</span></span>
          </div>

          <h1 className="hero-title" style={{ fontFamily:'var(--font-head)', fontSize:'clamp(2.8rem,5.5vw,5rem)', fontWeight:800, letterSpacing:'-0.025em', lineHeight:1.04, marginBottom:26, animation:'fadeUp 0.9s 0.2s ease both' }}>
          ...
          <div className="hero-stats" style={{ display:'flex', gap:48, marginTop:70, paddingTop:40, borderTop:'1px solid var(--border)', animation:'fadeUp 0.9s 0.65s ease both', flexWrap:'wrap' }}>
            {heroStats.map((st,i) => (
              <div key={i}><div className="stat-num">{st.value}</div><div className="stat-lbl">{st.label}</div></div>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="container">
          <div className="grid-2">
            <div className="reveal-left">
              ...
              <div className="about-mini-stats" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginTop:24 }}>
                {(hp.aboutMiniStats || [
                  ...
                  <div key={i} className={`reveal d${i+1}`} style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:8, padding:'22px 18px' }}>
                    <div style={{ fontFamily:'var(--font-head)', fontSize:'2rem', fontWeight:800, color:'var(--cyan)' }}>{st.value}</div>
                    <div style={{ fontSize:'0.75rem', color:'var(--muted)', textTransform:'uppercase', letterSpacing:'0.07em', marginTop:3 }}>{st.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="reveal-right">
              <div className="section-label">{hp.aboutSectionLabel || 'About Asproite'}</div>
              ...
              <div style={{ marginTop:36, display:'flex', gap:14, flexWrap:'wrap' }}>
                <Link to="/about" className="btn-primary">{hp.aboutPrimaryBtn || 'Learn About Us →'}</Link>
                <Link to="/contact" className="btn-ghost">{hp.aboutGhostBtn || 'Get a Quote'}</Link>
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ padding:'110px 0', background:'var(--bg2)', borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)' }}>
        <div className="container">
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:56 }} className="reveal section-flex-header">
            <div>
              <div className="section-label">{hp.servicesSectionLabel || 'What We Do'}</div>
              <h2>{hp.servicesSectionTitle || 'Our'} <em>{hp.servicesSectionTitleAccent || 'Core Services'}</em></h2>
            </div>
            <Link to="/services" className="btn-ghost">{hp.servicesLinkText || 'All Services →'}</Link>
          </div>
      ...
      {/* ── IT Health Check Banner ── */}
      <section style={{ padding:'0 0 80px' }}>
        <div className="container">
          <div className="reveal" style={{ background:'linear-gradient(135deg,#0a1628 0%,#061020 100%)', border:'1px solid rgba(0,212,255,0.2)', borderRadius:20, padding:'52px 60px', display:'flex', alignItems:'center', gap:48, flexWrap:'wrap', position:'relative', overflow:'hidden' }}>
            {/* glow */}
            <div style={{ position:'absolute', top:'-50%', right:'-10%', width:'40%', height:'200%', background:'radial-gradient(circle,rgba(0,212,255,0.07) 0%,transparent 70%)', pointerEvents:'none' }} />
            <div style={{ flex:1, minWidth:260, position:'relative', zIndex:1 }}>
              <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'rgba(0,212,255,0.08)', border:'1px solid rgba(0,212,255,0.2)', borderRadius:100, padding:'4px 14px', fontSize:'0.65rem', letterSpacing:'0.15em', textTransform:'uppercase', color:'var(--cyan)', marginBottom:16 }}>
                🩺 Free Tool
              </div>
              <h2 style={{ fontFamily:'var(--font-head)', fontSize:'clamp(1.4rem,2.8vw,2rem)', fontWeight:800, marginBottom:12, lineHeight:1.2 }}>
                Is your IT putting your<br /><em style={{ color:'var(--cyan)' }}>business at risk?</em>
              </h2>
              <p style={{ color:'var(--muted)', fontSize:'0.9rem', lineHeight:1.75, maxWidth:420, marginBottom:0 }}>
                Take our free 3-minute IT Health Check. Get an instant personalised score revealing your vulnerabilities, strengths, and exactly where to focus next.
              </p>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:14, position:'relative', zIndex:1, minWidth:220 }}>
              {['⚡ Reliability & Support','🔒 Security & Data Protection','☁️ Infrastructure & Cloud','📊 IT Strategy & Planning'].map(item => (
                <div key={item} style={{ display:'flex', alignItems:'center', gap:10, fontSize:'0.8rem', color:'var(--muted)' }}>
                  <div style={{ width:18, height:18, borderRadius:'50%', background:'rgba(0,255,136,0.15)', border:'1px solid rgba(0,255,136,0.3)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                    <div style={{ width:6, height:6, borderRadius:'50%', background:'#00ff88' }} />
                  </div>
                  {item}
                </div>
              ))}
              <Link to="/it-health-check" className="btn-primary" style={{ marginTop:8, textAlign:'center', justifyContent:'center' }}>
                Get My Free IT Score →
              </Link>
            </div>
          </div>
        </div>
      ...
      <CTABox
        label={hp.ctaLabel || 'Get Started'}
        title={hp.ctaTitle || 'Ready to'}
        titleAccent={hp.ctaTitleAccent || 'Elevate Your IT?'}
        subtitle={hp.ctaSubtitle || 'Talk to our team today and discover how Asproite can modernise, secure, and scale your technology.'}
        primaryText={hp.ctaPrimaryText || 'Get a Free Quote →'}
        primaryTo="/contact"
        ghostText={hp.ctaGhostText || 'See Our Work'}
        ghostTo="/portfolio" />
    </>
  );
}
