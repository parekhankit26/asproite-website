...
// ── Custom Cursor (Portal — GPU-composited via transform) ──
export function Cursor() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      const p = pos.current;
      // Use transform: translate3d — runs entirely on GPU, zero layout cost
      if (curRef.current) {
        curRef.current.style.transform = `translate3d(${p.mx - 10}px, ${p.my - 10}px, 0)`;
      }
      // Lerp trail smoothly
      p.tx += (p.mx - p.tx) * 0.10;
      p.ty += (p.my - p.ty) * 0.10;
      if (trailRef.current) {
        trailRef.current.style.transform = `translate3d(${p.tx - 18}px, ${p.ty - 18}px, 0)`;
      }
