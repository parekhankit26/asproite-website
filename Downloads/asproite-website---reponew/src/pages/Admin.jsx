...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
function SettingsSection({ allData }) {
  ...
  // Generates a complete ready-to-use api.js with current admin data baked in.
  // User just drops this file into src/data/ and redeploys — no code editing needed.
  const downloadApiJs = () => {
    if (!allData) { alert('No data loaded yet. Please wait a moment and try again.'); return; }

    const w3key = localStorage.getItem('asproite_web3key') || 'YOUR_WEB3FORMS_ACCESS_KEY';
    const dataJson = JSON.stringify(allData, null, 2);

    const fileContent = `// ═══════════════════════════════════════════════════════════
// ASPROITE — DATA API
// Auto-generated by Admin Panel on ${new Date().toLocaleString()}
// This file contains your published content baked in as defaults.
// Every device in the world will see this content after you deploy.
// ═══════════════════════════════════════════════════════════

const DB_KEY = 'asproite_db';
const ADMIN_PW_KEY = 'asproite_admin_pw';
const DEFAULT_PASSWORD = 'asproite2024';
// Bump CACHE_VERSION any time you want to force ALL browsers to reload fresh data
const CACHE_VERSION = 'published-${Date.now()}';
const CACHE_VERSION_KEY = 'asproite_cache_version';

// Force all browsers/devices to load the freshly published defaults
...
// All admin changes have been baked in below.
// This is what every visitor on every device will see.
function getDefaults() {
  return ${dataJson};
}

// ── Web3Forms key for contact form ───────────────────────
// Emails go to inquiry@asproite.com
export const WEB3FORMS_KEY = localStorage.getItem('asproite_web3key') || '${w3key}';

// ── Deep per-section merge ────────────────────────────────
function loadDB() {
  try {
...
`;
  ...
  const step = (n, title, desc, isCode) => (
    <div style={{ display: 'flex', gap: 14, marginBottom: 16 }}>
      <div style={{ width: 30, height: 30, borderRadius: '50%', background: C.cyan, color: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '0.85rem', flexShrink: 0, marginTop: 2 }}>{n}</div>
      <div style={{ flex: 1 }}>
        <div style={{ fontWeight: 600, fontSize: '0.88rem', marginBottom: 4 }}>{title}</div>
        {isCode
          ? <code style={{ display: 'block', background: C.bg, border: `1px solid ${C.border}`, borderRadius: 6, padding: '10px 14px', fontSize: '0.78rem', color: C.cyan, lineHeight: 1.7, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>{desc}</code>
          : <div style={{ color: C.muted, fontSize: '0.8rem', lineHeight: 1.55 }}>{desc}</div>
        }
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <p style={{ color: C.muted, fontSize: '0.85rem', marginBottom: 28, lineHeight: 1.6 }}>
        Publish your admin content to all devices worldwide, and manage your contact form email.
      </p>

      {/* ── WHY explanation ── */}
      <div style={{ background: 'rgba(0,212,255,0.04)', border: `1px solid ${C.cyan}25`, borderRadius: 10, padding: '18px 20px', marginBottom: 24, fontSize: '0.82rem', lineHeight: 1.75, color: C.muted }}>
        <strong style={{ color: C.cyan, display: 'block', marginBottom: 6 }}>⚠️ Why don't my changes show on mobile / other browsers?</strong>
        When you edit content in Admin, it's saved in <em>this browser's</em> local storage only. Other devices (your phone, other people's computers) load the <em>default content baked into the code</em>. The solution is simple: use the <strong style={{ color: C.text }}>Publish button below</strong> — it downloads a new <code>api.js</code> file with your content already inside. Replace the old file and deploy. Done.
      </div>

      {/* ── BIG PUBLISH BUTTON ── */}
      <div style={{ ...bCard, border: `2px solid ${C.cyan}50`, background: `linear-gradient(135deg, ${C.surface}, rgba(0,212,255,0.04))` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          <span style={{ fontSize: '2rem' }}>🚀</span>
          <div>
            <div style={{ fontFamily: font.head, fontWeight: 700, fontSize: '1.1rem' }}>Publish Content to All Devices</div>
            <div style={{ color: C.muted, fontSize: '0.8rem', marginTop: 2 }}>Downloads a ready-to-use <code style={{ color: C.cyan }}>api.js</code> file with all your content baked in</div>
          </div>
        </div>

        <button
          onClick={downloadApiJs}
          style={{
            width: '100%', padding: '16px 0', background: downloaded ? C.green : C.cyan,
            color: C.bg, border: 'none', borderRadius: 7, fontFamily: font.head,
            fontWeight: 800, fontSize: '1.05rem', cursor: 'pointer',
            transition: 'background 0.3s', marginBottom: 24, letterSpacing: '0.03em',
          }}>
          {downloaded ? '✅ api.js Downloaded! Follow steps below →' : '⬇️ Download api.js (Publish All Content)'}
        </button>

        <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 20 }}>
          <div style={{ fontSize: '0.78rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: C.muted, marginBottom: 16 }}>After downloading — 3 steps to go live:</div>
          {step(1, 'Replace the file in your project', 'Move the downloaded api.js into your project folder:\nsrc/data/api.js\n(overwrite / replace the existing file)')}
          {step(2, 'Build & deploy', 'npm run build\ngit add .\ngit commit -m "publish content update"\ngit push -u origin main --force', true)}
          {step(3, 'Done! ✓', 'Hostinger auto-deploys in ~2 minutes. Every device in the world — mobile, tablet, other browsers — will now show your updated content.')}
        </div>
      </div>
      ...
      <div style={bCard}>
        ...
        <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 7, padding: '12px 16px', marginBottom: 16, fontSize: '0.8rem', color: C.muted, lineHeight: 1.65 }}>
          Get your free key at <a href="https://web3forms.com" target="_blank" rel="noreferrer" style={{ color: C.cyan }}>web3forms.com</a> — enter <strong style={{ color: C.text }}>inquiry@asproite.com</strong> → click "Create Access Key". Paste it below, save, then click Publish above.
        </div>
        <label style={{ fontSize: '0.72rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: C.muted, display: 'block', marginBottom: 7 }}>Web3Forms Access Key</label>
        <div style={{ display: 'flex', gap: 10 }}>
          <input
            value={web3key}
            onChange={e => setWeb3key(e.target.value)}
            placeholder="e.g. xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
            style={{ flex: 1, background: C.bg, border: `1px solid ${web3key ? C.cyan : C.border}`, borderRadius: 5, padding: '11px 14px', color: C.text, fontFamily: font.body, fontSize: '0.88rem', outline: 'none', transition: 'border-color 0.2s' }}
          />
          <button onClick={saveKey} style={{ ...bG, padding: '11px 22px', whiteSpace: 'nowrap', fontWeight: 700 }}>
            {keySaved ? '✓ Saved!' : 'Save'}
          </button>
        </div>
        {web3key && (
          <div style={{ marginTop: 10, fontSize: '0.78rem', color: C.green, display: 'flex', alignItems: 'center', gap: 6 }}>
            ✓ Key saved. <strong>Click "Publish" above</strong> to bake this key into the deployed code.
          </div>
        )}
      </div>
      ...
      <div style={bCard}>
        ...
        <div style={{ color: C.muted, fontSize: '0.8rem', marginBottom: 16 }}>Save a JSON copy of your admin data, or load a previous backup into this browser.</div>
