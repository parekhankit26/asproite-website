...
import { services, techStack } from '../data/siteData.js';
...
function FloatBadge({ icon, val, label, style }) {
  return (
    <div style={{
      ...
      background: 'rgba(11,16,25,0.88)', border: '1px solid var(--border)',
      borderRadius: 8, padding: '12px 18px', backdropFilter: 'blur(16px)',
      display: 'flex', alignItems: 'center', gap: 10,
      animation: 'floatBadge 4s ease-in-out infinite', ...style,
    }}>
...
const TYPING_PHRASES = [
  'UK IT Managed Service Provider',
  'Cloud & AI Experts',
  '25+ Years of Excellence',
  'Hardware Decommissioning Experts',
];
  ...
  return (
    <>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '120px 0 80px', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 750, padding: '0 64px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-head)', fontSize: 'clamp(2.8rem,5.5vw,5rem)', fontWeight: 800, letterSpacing: '-0.025em', lineHeight: 1.04, marginBottom: 26, animation: 'fadeUp 0.9s 0.2s ease both' }}>
            Transform Your<br />
            <span style={{ color: 'var(--cyan)' }}>Digital Future</span>
          </h1>
          ...
          <p style={{ fontSize: '1.05rem', lineHeight: 1.75, color: 'var(--muted)', maxWidth: 560, marginBottom: 44, animation: 'fadeUp 0.9s 0.35s ease both' }}>
            Asproite delivers end-to-end IT solutions — from cloud infrastructure and AI to hardware decommissioning. Trusted by private and public sector organisations across the UK for over 25 years.
          </p>

          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', animation: 'fadeUp 0.9s 0.5s ease both' }}>
          ...
          <div className="grid-2">
            <div className="reveal-left">
              <div style={{ position: 'relative', maxWidth: 460, aspectRatio: '1/1' }}>
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(circle at center, rgba(0,212,255,0.07) 0%,transparent 65%)', border: '1px solid var(--border)', borderRadius: 12, fontSize: '7rem', position: 'relative', overflow: 'hidden' }}>
            ...
            <div className="reveal-right">
              ...
              <h2 style={{ marginBottom: 20 }}>The UK's Most Trusted <em>IT Partner</em></h2>
              <p style={{ color: 'var(--muted)', lineHeight: 1.8, marginBottom: 14, fontSize: '0.95rem' }}>Welcome to Asproite Cloud and Consultancy — a dynamic IT solutions provider committed to delivering cutting-edge services that drive your business forward.</p>
              <p style={{ color: 'var(--muted)', lineHeight: 1.8, marginBottom: 24, fontSize: '0.95rem' }}>From IT support and software solutions to web development, mobile apps, AI strategies, and responsible hardware decommissioning — our experts craft innovative, scalable digital solutions.</p>
              {['ISO-certified processes & wide-reaching technology accreditations','24/7 dedicated helpdesk and infrastructure monitoring','Offices in London, UK and Vadodara, India','Full lifecycle IT — procurement to decommissioning'].map(item => (
      ...
      <section style={{ padding: '110px 0', background: 'var(--bg2)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
        <div className="container">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 56 }} className="reveal">
            ...
            <Link to="/services" className="btn-ghost">All 9 Services →</Link>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 2 }}>
            {services.slice(0, 6).map((s, i) => (
              <ServiceCard key={s.id} service={s} delay={i % 3} />
            ))}
          </div>
        </div>
      </section>
      ...
      <CTABox
        label="Get Started"
        title="Ready to"
        titleAccent="Elevate Your IT?"
        subtitle="Talk to our team today and discover how Asproite can modernise, secure, and scale your technology — including our brand-new Hardware Decommissioning service."
        primaryText="Get a Free Quote →"
        primaryTo="/contact"
        ghostText="See Our Work"
        ghostTo="/portfolio"
      />
    </>
  );
...
function ServiceCard({ service, delay }) {
  return (
    <div className={`reveal d${delay}`} style={{ background: 'var(--bg)', padding: '36px 32px', position: 'relative', overflow: 'hidden', transition: 'background 0.3s, transform 0.3s', border: '1px solid transparent', cursor: 'default', ...(service.isNew ? { borderColor: 'rgba(0,212,255,0.2)' } : {}) }}
      ...
      <Link to="/services" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--cyan)', fontSize: '0.74rem', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', textDecoration: 'none', marginTop: 16, transition: 'gap 0.2s' }}>Learn More →</Link>
    </div>
  );
}
