...
export default function Portfolio() {
  ...
  const pp = data?.portfolioPage || {};
  useScrollReveal();
  const [activeFilter, setActiveFilter] = useState('all');

  const statsBar = pp.statsBar || [
  ...
  const filtered = useMemo(() =>
    activeFilter === 'all' ? portfolioProjects : portfolioProjects.filter(p => p.category === activeFilter),
    [activeFilter, portfolioProjects]
  );

  return (
    <>
      <style>{`@keyframes scanLines { from{background-position:0 0} to{background-position:0 4px} }`}</style>

      <PageHeader
        title={pp.pageTitle || 'Our'}
        titleAccent={pp.pageTitleAccent || 'Portfolio'}
        breadcrumb="Portfolio"
        subtitle={pp.subtitle || 'A selection of projects showcasing our expertise across web development, cloud solutions, AI, mobile apps, and more.'}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', flexWrap: 'wrap', gap: 40 }}>
            {statsBar.map((s, i) => (
              <div key={i} style={{ textAlign: 'center' }}>
                <div style={{ fontFamily: 'var(--font-head)', fontSize: '2.6rem', fontWeight: 800, color: 'var(--cyan)' }}>{s.value}</div>
                <div style={{ fontSize: '0.78rem', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.09em', marginTop: 4 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: '100px 0', background: 'var(--bg2)', borderTop: '1px solid var(--border)' }}>
        <div className="container">
          <div className="reveal" style={{ textAlign: 'center', marginBottom: 60 }}>
            <SectionHeader
              label={pp.testimonialsSectionLabel || 'Client Feedback'}
              title={pp.testimonialsSectionTitle || 'What Clients'}
              titleAccent={pp.testimonialsSectionTitleAccent || 'Say'}
              center />
          </div>
      ...
      <CTABox
        label={pp.ctaLabel || 'Start a Project'}
        title={pp.ctaTitle || 'Your Project Could Be'}
        titleAccent={pp.ctaTitleAccent || 'Next'}
        subtitle={pp.ctaSubtitle || "Ready to build something extraordinary? Let's talk about your goals."}
        primaryText={pp.ctaPrimaryText || 'Start a Conversation →'}
        primaryTo="/contact"
        ghostText={pp.ctaGhostText || 'Our Services'}
        ghostTo="/services" />
    </>
  );
}
