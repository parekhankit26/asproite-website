...
export default function Services() {
  ...
  const processSteps = sp.processSteps || [
  ...
  const whyCards = sp.whyCards || [
    { icon: '⚡', title: '24/7 Support', body: 'Round-the-clock helpdesk means your business never stops.' },
    { icon: '🏆', title: 'Certified', body: 'Wide-reaching technology accreditations and vendor partnerships.' },
    { icon: '🌐', title: 'Full Lifecycle', body: 'From procurement and deployment to secure decommissioning.' },
    { icon: '🔒', title: 'Security First', body: 'Security baked into every solution we deliver, not bolted on.' },
  ];

  return (
    <>
      <PageHeader
        title={sp.pageTitle || 'Our'}
        titleAccent={sp.pageTitleAccent || 'Services'}
        breadcrumb="Services"
        subtitle={sp.subtitle || '9 specialised IT services covering every stage of your digital journey.'}>
        <CircuitCanvas />
      </PageHeader>
      ...
      <section style={{ padding: '100px 0' }}>
        <div className="container">
          <div className="reveal" style={{ textAlign: 'center', marginBottom: 60 }}>
            <SectionHeader
              label={sp.sectionLabel || 'What We Offer'}
              title={sp.sectionTitle || 'Complete'}
              titleAccent={sp.sectionTitleAccent || 'IT Solutions'}
              center />
          </div>
      ...
      <section style={{ padding: '100px 0', background: 'var(--bg2)', borderTop: '1px solid var(--border)' }}>
        <div className="container">
          <div className="reveal" style={{ textAlign: 'center', marginBottom: 64 }}>
            <SectionHeader
              label={sp.processSectionLabel || 'How We Work'}
              title={sp.processSectionTitle || 'Our'}
              titleAccent={sp.processSectionTitleAccent || 'Process'}
              center />
          </div>
          <div className="process-steps-grid" style={{ display: 'grid', gridTemplateColumns: `repeat(${processSteps.length},1fr)`, gap: 0, position: 'relative' }}>
            ...
            {processSteps.map((s, i) => (
              <div key={s.id||i} className={`reveal d${i}`} style={{ textAlign: 'center', padding: '0 16px' }}>
        ...
        <div className="container">
          <div className="grid-2">
            <div className="reveal-left">
              <div className="section-label">{sp.whySectionLabel || 'Why Asproite'}</div>
              <h2 style={{ marginBottom: 16 }}>{sp.whySectionTitle || 'The Reasons Clients'} <em>{sp.whySectionTitleAccent || 'Choose Us'}</em></h2>
              <p style={{ color: 'var(--muted)', lineHeight: 1.75, fontSize: '0.95rem', marginBottom: 32 }}>{sp.whyText || "Over 25 years we've built a reputation on delivering what we promise."}</p>
              <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                <Link to="/contact" className="btn-primary">{sp.whyPrimaryBtn || 'Start a Project →'}</Link>
                <Link to="/portfolio" className="btn-ghost">{sp.whyGhostBtn || 'See Portfolio'}</Link>
              </div>
            </div>
            <div className="why-cards-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              {whyCards.map((w, i) => (
                <div key={w.id||i} className={`reveal d${i % 2 + 1}`} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: 28, display: 'flex', gap: 18, transition: 'border-color 0.3s, transform 0.3s' }}
                  ...
                  <div>
                    <div style={{ fontFamily: 'var(--font-head)', fontSize: '0.95rem', fontWeight: 600, marginBottom: 6 }}>{w.title}</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--muted)', lineHeight: 1.65 }}>{w.body}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <CTABox
        label={sp.ctaLabel || "Ready to Start?"}
        title={sp.ctaTitle || "Let's Build Something"}
        titleAccent={sp.ctaTitleAccent || 'Remarkable'}
        subtitle={sp.ctaSubtitle || "Tell us about your project and we'll put together a tailored proposal."}
        primaryText={sp.ctaPrimaryText || 'Get a Free Quote →'}
        primaryTo="/contact"
        ghostText={sp.ctaGhostText || 'View Our Work'}
        ghostTo="/portfolio" />
    </>
  );
}
